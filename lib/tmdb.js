...
// Stremio requires `genres` as an array of STRINGS. TMDB details carry
// `genres` as [{id, name}] objects, so always normalize to names.
function genreNames(details, genreMap) {
  if (Array.isArray(details.genres) && details.genres.length) {
    if (typeof details.genres[0] === 'object') {
      return details.genres.map((g) => g && g.name).filter(Boolean);
    }
    return details.genres.filter((g) => typeof g === 'string');
  }
  return (details.genre_ids || []).map((g) => genreMap[g]).filter(Boolean);
}
